...
// This line is setting our api key variable to the environment variable we exported to our terminal. The api key is a unique identifier supplied by the api to prove we are who we say we are and it came from their credential managment page.
...
app.get('/api/v1/books/find', (req, res) => {
  ...
  // The query is set to an empty string then depending on the route clicked on by the user is populated by concatinating the results.
  let query = '';
  ...
  // Superagent is middleware intended to proxy queries for us. it uses AJAX calls to abstract things on the server side from the client side.
  superagent.get(url)
    .query({'q': query})
    .query({'key': API_KEY})
    .then(response => response.body.items.map((book, idx) => {
      ...
      // Destructuring is a shorthand notation for assigning many properties at the same time.
      let { title, authors, industryIdentifiers, imageLinks, description } = book.volumeInfo;

      // COMMENT: What is the purpose of the following placeholder image?
      // It holds space on the page so the dynamic styking will give the correct view even when nothing is selected.
      let placeholderImage = 'http://www.newyorkpaddy.com/images/covers/NoCoverAvailable.jpg';

      // COMMENT: Explain how ternary operators are being used below.
      // If the propertiy exists set the property to that property. If nothing was given for that property set the property to the appropriate 'not available'
      return {
...
// This route has a contextual paramiter. The :isbn is a variable container that holds what was captured when the route was activated.
